          ...
          <template #item="{ element: outElement }">
            <div
              ...
              :class="{
                ...
                drag,
                ['has-slot']: !!Object.keys(outElement.props.slots || {})
                  .length,
              }"
...
import {
  $$dropdown,
  DropdownOption,
} from "@/visual-editor/utils/dropdown-service";
...
watchEffect(() => {
  ...
  const isExistContent = firstCssRule.cssText.includes(
    ".simulator-editor-content"
  );
  if (isExistContent) {
    ...
    content: () => (
      <>
        <DropdownOption
          ...
          {...{
            onClick: () => {
              const index = parentBlocks.findIndex(
                (item) => item._vid == block._vid
              );
              if (index != -1) {
                const setBlockVid = (block: VisualEditorBlockData) => {
                  ...
                  if (slotKeys.length) {
                    slotKeys.forEach((slotKey) => {
                      slots[slotKey]?.children?.forEach((child) =>
                        setBlockVid(child)
                      );
                    });
                  }
